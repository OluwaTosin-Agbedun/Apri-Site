    ...
    <header className="sticky top-0 z-40 border-b border-hairline bg-background/90 backdrop-blur-md">
      <div className="max-w-5xl lg:max-w-6xl mx-auto px-6">
        <div className="flex items-center justify-between h-24">
          ...
          >
            <span className="font-serif text-lg text-foreground tracking-tight leading-none">
              APRI
            </span>
          ...
          <button
            ...
            className="xl:hidden text-sm text-foreground/70 hover:text-foreground transition-colors cursor-pointer"
          >
            {isOpen ? "Close" : "Menu"}
          </button>
        </div>
        ...
        {isOpen && (
          <nav className="xl:hidden pb-6 flex flex-col gap-4 border-t border-border pt-6">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
