 ...
 */
function requestHref(type?: string) {
  if (BRIEFING_FORM_URL) return BRIEFING_FORM_URL;
  return type ? `/request-briefing?type=${encodeURIComponent(type)}` : '/request-briefing';
}
...
function RequestLink({
  ...
  return (
    <Link to={requestHref(type)} className={className}>
      {label}
      {variant === 'quiet' && arrow}
    </Link>
  );
