...
export default async function EditSubscriberPage({
  ...
  if (id === "new") {
    return (
      ...
      >
        <SubscriberForm draft={BLANK} canDelete={false} />
      </AdminShell>
    )
  }
    ...
    >
      <div className="mb-6 border border-border bg-card/30 p-6">
        ...
        <p className="mt-4 pt-4 border-t border-border text-xs text-muted-foreground leading-relaxed max-w-xl">
          Complete the subscription access level, term end date and personal
          Papermark library link below, then click Save. The activation button
          refreshes immediately after a successful save.
        </p>
      </div>

      <SubscriberForm draft={draft} canDelete={admin.role === "owner"} />
    </AdminShell>
  )
}
