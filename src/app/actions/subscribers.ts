...
const SubscriberAdminSchema = z.object({
  ...
  publicTier: z.enum(PUBLIC_TIER_NAMES as [string, ...string[]]).or(z.literal('')),
...
export async function activateSubscriber(id: string): Promise<FormState> {
  ...
  if (!isLevel(row.level)) {
    return { message: 'Set an access level on this record before activating it.' }
  }
  if (!row.term_end) {
    return { message: 'Set a term end date before activating this seat.' }
  }
  if (new Date(row.term_end) < startOfToday()) {
    return { message: 'That term end date is in the past. Extend it before activating.' }
  }
...
export async function resendSignInLink(id: string): Promise<FormState> {
  ...
  try {
    ...
    return { message: 'Could not send the email. Check the email configuration.' }
  }
...
export async function setPublicationAccess(
  ...
  if (!parsed.success) return { message: 'That link must be an https:// address.' }
 ...
 */
export async function sendPublishAlert(publicationId: string): Promise<FormState> {
  await requireAdmin()
